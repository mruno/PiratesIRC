...
alias EPirate.Bot.Game.roulette.Start {
  ...
  var %player $1
  if ($timer(epirate.raid.interactive)) { epirate.msg %player Tis not be the time fer tis as we be raidin'! | return }
...
alias russian.roulette.start {
  ...
  if ($EPirate.ChannelBot($2)) || ($EPirate.IsClone($2)) { epirate.notice %player1 $2 cannot do this. | return }
    ...
    if (%player1 != $epirate.bot) {
      ;if ($hget(epirate.daily,Games.Played. $+ %player1) >= $EPirate.Max.Games.Per.Day) { epirate.notice %player1 Ye played enough games today! New day starts $EPirate.Time.Until.New.Day | return }
      if ($hget(EPirate.Expenses.Today,%player1 $+ .games) > $epirate.player.percentage(%player1,$EPirate.Max.Percentage.Daily.Games)) { epirate.notice %player1 Ye 'ave earned too much doubloons from games today! New day starts $EPirate.Time.Until.New.Day | return }
    }
    ;if ($hget(epirate.daily,Games.Played. $+ %player2) >= $EPirate.Max.Games.Per.Day) { epirate.notice %player2 %player2 Ye played enough games today! New day starts $EPirate.Time.Until.New.Day | return }
    if ($hget(EPirate.Expenses.Today,%player2 $+ .games) > $epirate.player.percentage(%player2,$EPirate.Max.Percentage.Daily.Games)) { epirate.notice %player1 %player2 earned too much from games today! New day starts $EPirate.Time.Until.New.Day | return }
    ...
    if (%wager < 1) || (- isin %wager) || (. isin %wager) || ($len(%wager) > 40) { epirate.notice %player1 I don't like that wager. | return }
    ...
    if (%wager > $epirate.player.percentage(%player2,$EPirate.Max.Percentage.Wager)) {
      ...
      epirate.notice %player2 Wager from %player1 be too high fer ye $+ ! Try $EPirate.Round($v2) doubloons or less.
      return
    }
    ...
    .timerepirate.playgame.check. $+ %player1 1 45 EPirate.Task.Check %player1 play.game
    ...
    if (!%rr.status) {
      ...
      epirate.Daily.Goal.Check %player1 game
      hinc EPirate.Stats ship.games
      EPirate.Achievement %player2 game
      epirate.Daily.Goal.Check %player2 game

      var %time 90
      if (%player1 == $epirate.bot) inc %time 200
      .timerRussian.rr.timeout 1 %time russian.rr.timeout
      var %r $rand(1,3)
      if (%r == 1) russian.rr.msg %player1.nickname %player1 pulls out a musketoon n' points it at %player2.nickname $2 $+ ...
      elseif (%r == 2) russian.rr.msg %player1.nickname %player1 starts a game o' !Pirate Roulette wit' %player2.nickname $2 $+ !
      elseif (%r == 3) russian.rr.msg %player1.nickname %player1 pulls out a musket, places one round in it, n' spins the cylinder... Pirate Roulette!
