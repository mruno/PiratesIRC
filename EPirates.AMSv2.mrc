  ...
  if ($1 == !p) || ($1 == !pirate) || ($1 == !pirates) {
    if $2 == ams || $2 == amssm {
      ...
      if $3 {
        ...
        if (%start) {
          if ($timer(epirate.raid.interactive)) { epirate.msg %ams-p1 Tis not be the time fer tis as we be raidin'! | return }
...
alias EPirate.Bot.Game.AMS.Start {
  ...
  if (!%ams-status) || (%ams-status == idle) noop
  else return
...
alias epirate.ams.start {
  ...
  if ($3) {
    ;ams cooldown
    if ($timer(EPirate.AMS.Cooldown).secs) { epirate.notice $1 Game bein' rigged... $duration($ifmatch) | return }

    ;too many games played today check
    if ($hget(EPirate.Expenses.Today,$1 $+ .games) > $epirate.player.percentage($1,$EPirate.Max.Percentage.Daily.Games)) { epirate.notice $1 Ye 'ave earned too much from games today! New day starts $EPirate.Time.Until.New.Day | return }
    ...
    if (!%ams-status) || (%ams-status == idle) {
      ...
      if ($3 ison %chan) && ($EPirate.Onboard($3)) || ($3 == $epirate.bot) {
        ...
        if (%ams-status) || (%ams-status == idle) {
          ...
          if ($hget(EPirate.Expenses.Today,%ams-p2 $+ .games) > $epirate.player.percentage(%ams-p2,$EPirate.Max.Percentage.Daily.Games)) { epirate.notice $1 %ams-p2 has earned too much from games today! New day starts $EPirate.Time.Until.New.Day | return }
...
alias ams-help {
  if ($1) {
    if ($timer(epirate.ams.help. $+ $1)) return
    .timerepirate.ams.help. $+ $1 1 60 noop
    ...
    epirate.notice $1 $ams-logo - $amsb(!p amssm <pirate> wager) to challenge a pirate to a game of $ams-logo
...
alias ams-announcewinner {
  ...
  if (%ams-p1 != $epirate.bot) {
    ...
    epirate.Daily.Goal.Check %ams-p1 game
  ...
  if (%ams-p2 != $epirate.bot) {
    ...
    epirate.Daily.Goal.Check %ams-p2 game
  ...
  .timerEPirate.AMS.Cooldown 1 30 noop
  ams-reset
}
alias ams-cancel {
  if (!$1) {
