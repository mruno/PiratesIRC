...
;Legacy code and Non-Network settings in C:\EP\elitepirates3.mrc
;Extra code in C:\EP\elitepirates4.mrc
;Extra Extra code in C:\EP\elitepirates5.mrc
;HTML in C:\EP\elitepirates.html.mrc
;--------------------------------
;"Season" is synonymous with "round"
;----Global SETTINGS --------------------------------------------------
...
alias EPirate.Auto.Restart.mIRC.Every.Days return 30
...
alias epirate.standard.topic {
  ...
  var %total $hget(epirate.players.all,0).item
  if ($hget(epirate.options,Next.Round.Loop)) && (%winner) {
    var %round $hget(epirate.options,round)
    var %more $calc($Epirate.Mininum.Players - %total)
    if (%total < $Epirate.Mininum.Players) set %topic %winner has won season %round o' Pirates! %more $iif(%more > 1,pirates,pirate) needed to get underway! 8!Pirates 11http://tiny.cc/PiratesIRC
    else set %topic %winner has won season %round o' Pirates! Next season starts 'bout $hget(epirate.options,Next.Round.Loop) hours! 8!Pirates 11http://tiny.cc/PiratesIRC
  }
  else {
    var %more $calc($Epirate.Mininum.Players - %total)
    ;($calc($ctime - $hget(EPirate.Options,start)) > 2592000) && 
    if (%total < $Epirate.Mininum.Players) && ($hget(EPirate.Options,round) > 1) set %topic Welcome aboard! !Pirates ( http://tiny.cc/PiratesIRC ) %more more $iif(%more > 1,pirates,pirate) needed to keep ship afloat!
    else set %topic Welcome aboard $epirate.ship $+ ! - Join wit' !Pirates $iif($gettok($EPirate.Holiday.Bonus,1,44),- 8Happy $ifmatch) - 11http://tiny.cc/PiratesIRC

    ;else set %topic !Pirates Game - Season $hget(EPirate.options,round) $+  - Join wit' !Pirates - $iif($gettok($EPirate.Holiday.Bonus,1,44),8Happy $ifmatch) 15Rate: $+ $EPirate.Doubloon.Rate 11http://tiny.cc/PiratesIRC
  }
  return $EPirate.Random.Topic.Colors($iif(%msg,%msg -) %topic)
}

;random set to mirc's $rand or dicelib.dll
alias Epirate.Random return mirc
alias EPirate.Banned.Player.Names return decline yes aye accept nay captain market ship captjack captkirk captainjack captainkirk bot mutiny none
alias EPirate.Level.1.Requirement return 10001
alias EPirate.Bonus.NewPlayer return $calc($EPirate.Level.1.Requirement * 1.10)
...
alias EPirate.Max.Lockpicked.Per.Day return $iif(%EPirate.End,999,5)
...
alias EPirate.Max.Rob.Capn.Per.Day return $iif(%EPirate.End,999,18)
...
alias Epirate.Port.Control.Reward.Percentage return 10
alias Epirate.Port.Protection.Taxes.Hourly.Percentage return 0.05
alias Epirate.Port.Protection.Taxes.Hourly.Percentage.CAP return 2.5
...
alias EPirate.Normal.Stamina {
  var %stamina 6
  if ($asctime(ddd) == Mon) set %stamina 4
  elseif ($asctime(ddd) == Sun) set %stamina 10
  return %stamina
}
alias EPirate.doubloon.Level.CMDS return 0.5
alias EPirate.Max.Duel.Bonus return $iif(%EPirate.End,999,5)
alias EPirate.Flood.Warn return 7
alias EPirate.Flood.Punish return 12
alias EPirate.Flood.Brig.Time return 2
...
alias EPirate.Bounty.Min.Level return 5
...
alias EPirate.Player.Quest.Per.Day return $iif($asctime(ddd) == Sun,3,1)
...
alias EPirate.Captain.Ship.Encounter.Doubloon.Rate return 1
...
alias epirate.Low.Level.CMDs.Cooldown return $rand(5,30)
...
alias Epirate.Skill.List return Charisma,Defense,Fishing,Luck,Meditation,Marksmanship,Swordsmanship,Thievery
alias Epirate.Sharpen.Power return 3
alias Epirate.BlackPowder.Power return 3
alias EPirate.Player.Level.Before.Zonbi return 5
...
alias EPirate.Sail.Time.Increments.Standard return 8
...
alias EPirate.Global.Max.Skills return $calc($EPirate.Max.Skills * 2)
...
;Port Investments
alias epirate.port.investments.list return brothel,market,shipwright,tavern
...
alias epirate.Brothel.investment.cost return 1.5
alias epirate.Brothel.Investment.Return return 0.25
alias epirate.Tavern.investment.cost return 3
alias epirate.Tavern.Investment.Return return 0.5
alias epirate.Market.investment.cost return 4.5
alias epirate.Market.Investment.Return return 0.75
alias epirate.ShipWright.investment.cost return 6
alias epirate.Shipwright.Investment.Return return 1
alias epirate.unnecessary.purchase.limit return 12
alias epirate.Max.Independent.Ports return 2
alias epirate.max.loops return $calc($calc($hget(epirate.players.all,0).item * 2) + 3)
alias epirate.round.bonus.player.max.level return 11
alias epirate.max.goals.per.day return 4
alias epirate.goal.min.level return 6
alias epirate.player.rewards.max return 5
alias epirate.autosail.after.moored.for.so.many.hours return 4
alias epirate.max.global.treasures return 2
alias epirate.max.global.treasure.expires.hours return $rand(4,6)
alias epirate.max.global.treasure.max.per.day return 3
alias epirate.piratehaven.port.visit.bonus.hours return 8
alias EPirate.Max.Bite.Per.Day return 24
alias Epirate.Solo.Players.Can.Sail.Mins return 60
alias epirate.teleportstone.uses.per.day return 3
alias epirate.port.Size.Small return 3
alias epirate.port.Size.Medium return 6
alias epirate.port.Size.Large return 10
alias epirate.ship.health.broke return 50
alias epirate.ship.regenerate.health.per.hour return 60
alias epirate.ship.min.cannons return 2
alias epirate.ship.max.cannons return 6
alias epirate.ship.max.shot.exploding return 10
alias epirate.ship.max.shot.canister return 10
alias epirate.ship.max.shot.chain return 10
alias epirate.shipwright.price.cannon return 20
alias epirate.shipwright.price.shot.exploding return 1
alias epirate.shipwright.price.shot.canister return 1
alias epirate.shipwright.price.shot.chain return 1
alias epirate.shipwright.price.ship.repair.1.percent return 0.05
alias epirate.shipwright.price.clean return 3
alias epirate.shipwright.price.grease return 2
...
alias EPirate.Auto.Captcha.Triggers {
  if ($Epirate.Captcha.Level == 1) return 100
  elseif ($Epirate.Captcha.Level == 2) return 300
  else return 600
}
